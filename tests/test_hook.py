...
def test_connection_type_registered():
    ...
    assert types["entropydata"] == "datacontract_provider.hooks.entropy_data.EntropyDataHook"


def test_hook_class_attributes():
    assert EntropyDataHook.conn_type == "entropydata"
...
def test_hook_defaults_and_publish_url():
    conn = Connection(conn_id="entropy_data", conn_type="entropydata", password="key")
...
def test_hook_custom_host():
    conn = Connection(conn_id="entropy_data", conn_type="entropydata", host="eu.example.com", password="key")
    with patch.object(EntropyDataHook, "get_connection", return_value=conn):
        hook = EntropyDataHook("entropy_data")
